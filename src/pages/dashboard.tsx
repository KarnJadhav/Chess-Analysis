...
function formatEvalDrop(evalDrop: number): string {
  if (evalDrop >= 20000) return 'Mate';
  return `${evalDrop} cp`;
}

function averageMoveAccuracy(
  moves: Array<{ side: 'white' | 'black'; moveAccuracy?: number }> | undefined,
  side: 'white' | 'black'
): number | null {
  if (!moves || moves.length === 0) return null;
  const filtered = moves.filter((move) => move.side === side && typeof move.moveAccuracy === 'number');
  if (filtered.length === 0) return null;
  const sum = filtered.reduce((acc, move) => acc + (move.moveAccuracy ?? 0), 0);
...
export default function Dashboard() {
  ...
  const whiteName = selectedGame?.whitePlayer || session?.user?.name || session?.user?.email || 'White';
  const blackName = selectedGame?.blackPlayer || selectedGame?.opponent || 'Black';
  ...
  async function handleUpload(e: React.FormEvent) {
    ...
    try {
      ...
      setAnalyzing(true);
      setMessage('Game uploaded. Running analysis on the server...');

      const saveRes = await fetch(`/api/games/analysis?gameId=${encodeURIComponent(uploadedGameId)}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({ gameId: uploadedGameId, runServerAnalysis: true }),
      });
  ...
  const derivedWhiteAccuracy = averageMoveAccuracy(reviewedMoves, 'white');
  const derivedBlackAccuracy = averageMoveAccuracy(reviewedMoves, 'black');
  const whiteAccuracyDisplay = playerSummary?.white.accuracy ?? selectedAnalysis?.accuracyWhite ?? derivedWhiteAccuracy ?? null;
  const blackAccuracyDisplay = playerSummary?.black.accuracy ?? selectedAnalysis?.accuracyBlack ?? derivedBlackAccuracy ?? null;

  const turningPoint = reviewedMoves
    .slice()
    .sort((left, right) => right.evalDrop - left.evalDrop)[0];
  const turningPointMove = turningPoint?.moveNumber;
  ...
  const brilliantMoveSet = new Set(
    reviewedMoves
      .filter((move) => move.brilliant)
      .map((move) => move.moveNumber)
  );
  ...
  const chartData = reviewTimeline.length > 0
    ? reviewTimeline.map((point) => ({
      ...
      isTurningPoint: turningPointMove === point.move,
      isBrilliant: brilliantMoveSet.has(point.move),
    }))
    : (selectedAnalysis?.moveEvals ?? []).map((evalScore, index) => ({
      ...
      isTurningPoint: turningPointMove === index + 1,
      isBrilliant: brilliantMoveSet.has(index + 1),
    }));
  ...
  function findNextCriticalMove(currentIndex: number): number | null {
    for (const move of reviewedMoves) {
      if (move.moveNumber <= currentIndex) continue;
      if (criticalMoveSet.has(move.moveNumber)) return move.moveNumber;
    }
    return null;
  }
  ...
  return (
    <div className="dashboard-root">
      ...
      {analyzing && (
        <div className="progress my-3 analyzing-progress" style={{ height: 24 }}>
          <div className="progress-bar progress-bar-striped progress-bar-animated bg-info analyzing-progress-bar" style={{ width: '100%' }}>
        ...
        <div className="row justify-content-center">
          <div className="col-12 col-lg-10">
            ...
            {viewerOpen && selectedGame && (
              <div className="modal fade show game-viewer-modal" style={{ display: 'block', background: 'rgba(8,12,20,0.6)' }} tabIndex={-1}>
                <div className="modal-dialog modal-xl modal-dialog-centered">
                  <div className="modal-content game-viewer-card">
                    <div className="modal-header game-viewer-header">
                      <div>
                        <div className="game-viewer-title">Game Review</div>
                        <div className="game-viewer-subtitle">{selectedGame.opening || 'Opening TBD'}</div>
                      </div>
                      <button type="button" className="btn-close" onClick={() => setViewerOpen(false)}></button>
                    </div>
                    <div className="modal-body game-viewer-body">
                      <div className="viewer-container">
                        <div className="board-container">
                          <div className="chess-board game-viewer-board">
                            ...
                            {activeReviewedMove ? (
                              <>
                                <div className="small mb-1">
                                  ...
                                  <span className="ms-2 text-muted">Move quality {activeReviewedMove.moveAccuracy ?? '-'}</span>
                                </div>
                                <div className="small text-muted mb-1">Best move: {activeReviewedMove.bestMove} | Eval drop: {formatEvalDrop(activeReviewedMove.evalDrop)}</div>
                                <div className="small">{activeReviewedMove.shortHint}</div>
                                {showDeepInsight && (
                                  <div className="small text-muted mt-2">{activeReviewedMove.longHint}</div>
                                )}
                                <button
                                  type="button"
                                ...
                                >
                                  {showDeepInsight ? 'Hide deep dive' : 'Deep dive'}
                                </button>
                            ...
                            {chartData.length > 0 ? (
                              <ResponsiveContainer width="100%" height="100%">
                                <LineChart data={chartData} margin={{ top: 12, right: 12, left: 4, bottom: 4 }}>
                                  ...
                                  {turningPointMove && (
                                    <ReferenceLine x={turningPointMove} stroke="#dc3545" strokeDasharray="4 4" />
                                  )}
                                  <Tooltip />
                                  <Line type="monotone" dataKey="eval" stroke="#0d6efd" strokeWidth={2} dot={false} />
                                  {chartData.map((point) => {
                                    const radius = point.isTurningPoint ? 8 : point.isCritical ? 6 : 4;
                                    const stroke = point.isTurningPoint ? '#212529' : point.dotColor;
                                    const strokeWidth = point.isTurningPoint ? 2 : 1;
                                    return (
                                      <ReferenceDot
                        ...
                        <div className="info-container">
                          <div className="game-info">
                            ...
                            <div className="game-info-row">
                              <span>Date</span>
                              <span>{selectedGame.date ? new Date(selectedGame.date).toLocaleDateString() : '-'}</span>
                            </div>
                          </div>

                          <div className="ai-summary">
                            <div className="ai-summary-title">AI Coach Summary</div>
                            {aiReview ? (
                              <>
                                {aiReview.coachMessage && (
                                  <div className="ai-summary-message">{aiReview.coachMessage}</div>
                                )}
                                <div className="ai-summary-text">{aiReview.summary}</div>
                              ...
                              <div className="ai-summary-warning">{aiReviewMessage || `AI review failed: ${aiReviewError || 'Unknown error'}`}</div>
                            ) : aiReviewStatus === 'skipped' ? (
                              <div className="ai-summary-muted">AI review skipped (missing API key).</div>
                            ) : (
                              <div className="ai-summary-muted">AI review unavailable. Add OpenRouter credits and click Regenerate.</div>
                            )}
                            <button
                              type="button"
                          ...
                          <div className="review-summary">
                            <div className="review-summary-title">Game Review</div>
                            {profileError && (
                              <div className="review-summary-warning">{profileError}</div>
                            )}
                            <div className="d-flex justify-content-between align-items-center mb-2">
                              ...
                              <div className="text-end">
                                <div className="fw-bold text-success">
                                  {whiteAccuracyDisplay === null ? '-' : `${whiteAccuracyDisplay}%`}
                                </div>
                            ...
                            <div className="d-flex justify-content-between align-items-center">
                              ...
                              <div className="text-end">
                                <div className="fw-bold text-success">
                                  {blackAccuracyDisplay === null ? '-' : `${blackAccuracyDisplay}%`}
                                </div>
                                <div className="small text-muted">
                            ...
                            {turningPoint && (
                              <div className="mt-3">
                                <div className="fw-semibold mb-2">Turning Point</div>
                                <div className="small mb-1">Move {turningPoint.moveNumber}: {turningPoint.san}</div>
                                <div className="small mb-2">
                                  <span
                                    className="badge"
                                    style={{ backgroundColor: classificationColors[turningPoint.classification] ?? '#6c757d' }}
                                  >
                                    {turningPoint.classification}
                                  </span>
                                  <span className="text-muted ms-2">Eval swing {formatEvalDrop(turningPoint.evalDrop)}</span>
                                </div>
                                <div className="small text-muted">{turningPoint.comment}</div>
                              </div>
                            )}
                          </div>
                          ...
                          <div className="mb-1">
                            ...
                            <button
                              ...
                              disabled={criticalMoveSet.size === 0}
                            >
                              Next Critical
                            </button>
                          ...
                          <div className="moves-list" ref={movesListRef}>
                            <ol className="mb-0">
                              {moves.map((move, idx) => {
                                ...
                                const isCritical = criticalMoveSet.has(idx + 1);
                                const isBrilliant = brilliantMoveSet.has(idx + 1) || reviewMove?.brilliant;
                                const isActive = idx === moveIndex - 1;
                                ...
                                const inlineHint = reviewMove?.shortHint;
                                return (
                                  <li
                                    key={idx}
                                    data-move-index={idx + 1}
                                    className={isActive ? 'fw-bold text-primary' : ''}
                                    style={isCritical ? { backgroundColor: '#fff3cd', borderRadius: 6, padding: '2px 6px' } : undefined}
                                  >
                                    <button
                                      type="button"
                                    ...
                                    >
                                      ...
                                      {isCritical && <span className="badge bg-warning text-dark ms-2">Critical</span>}
                                      {isBrilliant && <span className="badge bg-success ms-2">Brilliant</span>}
                                      {inlineHint && <span className="text-muted ms-2 small">{inlineHint}</span>}
                                    </button>
                                  </li>
                                );
                              })}
              ...
              <div className="row">
                ...
                <div className="col-12 col-md-4 mb-3">
                  <div className="bg-success bg-opacity-10 border border-success rounded-3 p-4 shadow-sm" style={{ minHeight: '200px' }}>
                    <h5 className="fw-bold text-dark mb-3">💡 Suggestions</h5>
                    {latestAiReview ? (
                      <div className="mb-3">
                        <div className="small fw-semibold text-dark">AI Coach</div>
                        <div className="small text-dark">{latestAiReview.summary}</div>
                        {latestAiReview.improvementSuggestions?.length > 0 && (
                          <ul className="small mt-2 mb-0 text-dark">
                    ...
                    <ul className="mb-0 text-dark" style={{ fontSize: '0.95rem', lineHeight: '1.6' }}>
                      {games.length === 0 ? (
                        <li className="text-dark">No suggestions yet.</li>
